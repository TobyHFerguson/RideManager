...
- [x] Task 7.7: Delete _transformV1ToWebFormat ✅ Commit f789bf6
- [x] Task 7.8: Clean SCCCCEvent - remove API artifacts from domain ✅
...
### Task 7.7: Evaluate _transformV1ToWebFormat (Decision Point) ✅ COMPLETE

**Decision**: Option 1 - Delete entirely

- [x] 7.7.1 Analyze consumers: Who needs starts_at? Who needs organizers array? → fromRwgpsEvent is only consumer
- [x] 7.7.2 Decide on approach → Delete and update fromRwgpsEvent to handle v1 format directly
- [x] 7.7.3 Implement decision (TDD)
  - Added 3 new tests for v1 format handling in EventFactory.test.js
  - Updated fromRwgpsEvent() to accept start_date+start_time and organizers array
- [x] 7.7.4 Run `npm test` - all 683 tests pass
- [x] 7.7.5 Commit: f789bf6 "Task 7.7: Delete _transformV1ToWebFormat - consumers now accept v1 directly"

### Task 7.8: Clean SCCCCEvent - Remove API Artifacts from Domain (TDD)

**Analysis of SCCCCEvent Field Usage:**

| Field | Domain Use | API Use | Action |
|-------|-----------|---------|--------|
| `name` | ✅ Used (ride names, cancellation) | ✅ Required | KEEP |
| `description` | ✅ Used (generated by EventFactory) | ✅ Required | KEEP |
...
3. **Updated EventFactory.fromRwgpsEvent** to stop setting API-only fields on SCCCCEvent
